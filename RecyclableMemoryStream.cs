public sealed class RecyclableMemoryStream : MemoryStream
{
    ...
    {
        if (Interlocked.CompareExchange(ref disposedState, 1L, 0L) != 0)
        {
    ...
    public override void Write(byte[] buffer, int offset, int count)
    {
        ...
        if ((num + blockSize - 1) / blockSize * blockSize > int.MaxValue)
        {
            modCommon.WriteLine("Error in RecyclableMemoryStream: Maximum capacity exceeded");
        }
    ...
    public override long Seek(long offset, SeekOrigin loc)
    {
        ...
        int num = loc switch
        {
            ...
            SeekOrigin.Current => (int)offset + position,
            SeekOrigin.End => (int)offset + length,
