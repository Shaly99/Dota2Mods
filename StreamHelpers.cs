...
public static class StreamHelpers
{
    ...
    public static async Task<byte[]> ReadAllBytesAsync(string filename)
    {
        ...
        using (FileStream fs = File.OpenRead(filename))
        {
            while (true)
            {
                ...
                int result3;
                int result2 = (result3 = taskAwaiter4.GetResult());
                int result4 = result3;
                if (result2 <= 0)
                {
                    break;
                }
                TaskAwaiter taskAwaiter5 = ms.WriteAsync(buffer, 0, result4).GetAwaiter();
    ...
    public static async Task<string> ReadAllTextAsync(string filename)
    {
        ...
        return encoding2.GetString(await ReadAllBytesAsync(filename).ConfigureAwait(continueOnCapturedContext: false));
    }
