...
public class frmTools : Form, IItemHost
{
    ...
    // ════════════════════════════════════════
    // Hook global para detectar clicks fuera
    // ════════════════════════════════════════
    ...
    public frmTools(Items items)
    {
        ...
        item = items;
        Application.OpenForms.OfType<Form>().SingleOrDefault((Form pre) => pre.Name == "frmLoading")?.Close();
        LoadItems();
    ...
    private void LoadItems()
    {
        try
        {
            // Obtener todos los items del mismo tipo/prefab del item recibido
            List<Items> list = ItemsManager.GetItemsFromPrefab(item);
    ...
    public void LoadStyles(Items item)
    {
        ...
        {
            string text = !string.IsNullOrEmpty(item.styles[i].icon_path)
                ? item.styles[i].icon_path
        ...
        try
        {
            ...
            if (nCode >= 0 && (wParam == (IntPtr)WM_LBUTTONDOWN || wParam == (IntPtr)WM_RBUTTONDOWN))
            {
                ...
                if (!this.Bounds.Contains(clickPoint))
                {
                    ...
                    // 🔑 Consumir el click para que no se propague (evita el sonido "ding")
                    return (IntPtr)1;
                }
            }
        }
    ...
    private void InitializeComponent()
    {
        ...
        base.ClientSize = new System.Drawing.Size(407, 323);
