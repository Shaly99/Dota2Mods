...
public static class KValueHelpers
{
    ...
    {
        if (int.TryParse(k.Value.ToString(), out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        if (long.TryParse(k.Value.ToString(), out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        if (uint.TryParse(k.Value.ToString(), out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        if (ulong.TryParse(k.Value.ToString(), out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        if (float.TryParse(k.Value.ToString(), NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        if (Enum.TryParse<T>(k.Value.ToString(), out var result))
        {
            return result;
        }
        return defaultValue;
    ...
    {
        foreach (KeyValuePair<char, char> item in EscapeMap)
        {
        ...
        if (k.Children != null)
        {
            foreach (KValue child in k.Children)
            {
                ...
                switch (child.KType)
                {
                    case KValueType.Color:
                        {
                            Color valueOrDefault = (child.Value as Color?).GetValueOrDefault();
                            WriteString(stream, "#" + valueOrDefault.R.ToString("X2") + valueOrDefault.G.ToString("X2") + valueOrDefault.B.ToString("X2"), quote: true);
                            break;
                        }
                    default:
    ...
    private static object ToDictionary(this KValue k, int depth)
    {
        ...
        if (flag)
        {
            ...
            foreach (KValue child in k.Children)
            {
                dictionary3[EscapeString(child.Name)] = child.ToDictionary(depth + 1);
            }
        ...
        if (k.Value == null)
        {
            foreach (KValue child2 in k.Children)
            {
                dictionary[EscapeString(child2.Name)] = child2.ToDictionary(depth + 1);
            }
            return dictionary;
        }
        if (k.Value is string)
    ...
    public static bool RemoveKey(this KValue k, string path)
    {
        ...
        string[] array = path.Split(new string[4] { "/", "|", ",", "." }, StringSplitOptions.RemoveEmptyEntries);
        if (array.Length == 0)
        {
            return false;
        }
